...
setLDAndVerifyTools () {
  gnuPath=/tools/$(uname -m)-pc-linux-gnu/bin &&
  if [ ! -L "$gnuPath"/ld ]; then
    mv -v /tools/bin/{ld,ld-old} &&
    mv -v "$gnuPath"/{ld,ld-old} &&
    cp -v /tools/bin/{ld-new,ld} &&
    ln -sv /tools/bin/ld "$gnuPath"/ld ||
    return
  fi &&
  ...
  coutput=$(readelf -l a.out | grep ': /lib') &&
  echo "$coutput" &&
